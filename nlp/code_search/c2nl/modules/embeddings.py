...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,  # pylint: disable=dangerous-default-value
                 word_vocab_size,
                 word_padding_idx,
                 position_encoding=False,
                 feat_merge="concat",
    ...
    def _validate_args(self, feat_merge, feat_vocab_sizes, feat_vec_exponent,
        ...
        if n_feats != len(feat_padding_idx):
            raise ValueError("Got unequal number of feat_vocab_sizes and "
                             f"feat_padding_idx ({n_feats} != {len(feat_padding_idx)})")
    ...
    def forward(self, source, step=None):
        ...
        """
        if self.position_encoding:
            for i, module in enumerate(self.make_embedding._modules.values()):  # pylint: disable=protected-access
                if i == len(self.make_embedding._modules.values()) - 1:  # pylint: disable=protected-access
