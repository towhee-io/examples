...
def process_examples(lang_id,
    ...
    TAG_TYPE_MAP = TOKEN_TYPE_MAP if code_tag_type == 'subtoken' else AST_TYPE_MAP  # pylint: disable=invalid-name
    code = Code()
    code.text = source
    ...
    if target is not None:
        ...
        if not test_split:
            ...
            pad_length = max_tgt_len - len(summ_tokens)
            summ_tokens += ['<blank>'] * pad_length
...
def load_data(args, filenames, max_examples=-1, dataset_name='java',
    ...
    for src, src_tag, tgt in tqdm(zip(sources, source_tags, targets),
        ...
        if dataset_name in ['java', 'python', 'codeSearchNet-py']:
            ex_ = process_examples(LANG_ID_MAP[DATA_LANG_MAP[dataset_name]],
                                   src,
                                   src_tag,
                                   tgt,
                                   args.max_src_len,
            ...
            print('dataset_name unregistered!')
            raise KeyError
...
def build_word_dict(examples, fields, dict_size=None,
                    no_special_token=False):
    """Return a dictionary from question and document words in
    provided examples.
    """
    word_dict = Vocabulary(no_special_token)
    for w in load_words(examples, fields, dict_size):
        word_dict.add(w)
    return word_dict
